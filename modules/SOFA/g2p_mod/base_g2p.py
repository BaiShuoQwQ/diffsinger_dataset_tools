...
class BaseG2P:
    ...
    def get_dataset(self, wav_paths, lab_folder):
        # dataset is a pandas dataframe with columns: wav_path, ph_seq, word_seq, ph_idx_to_word_idx
        dataset = []
        for wav_path in wav_paths:
            try:
                if (lab_folder / wav_path.name).with_suffix("." + self.in_format).exists():
                    with open(
                        (lab_folder / wav_path.name).with_suffix("." + self.in_format),
                        "r",
                        encoding="utf-8",
                    ) as f:
