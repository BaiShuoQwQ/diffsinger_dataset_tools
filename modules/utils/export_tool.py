...
class Exporter:
    ...
    def save_textgrids(self, out_folder):
        ...
        for (
            ...
            label_path = (
                    out_folder / wav_path.with_suffix(".TextGrid").name
            )
    ...
    def save_confidence_fn(self, out_folder):
        ...
        for (
            ...
            name = wav_path.with_suffix("").name
            curr_data = {
            "name": wav_path.with_suffix("").name,  # 直接赋值，而不是使用列表
        ...
        df.to_csv(out_folder / "confidence.csv", index=False)

    def export(self, out_formats, out_folder):
        if "textgrid" in out_formats or "praat" in out_formats:
            self.save_textgrids(out_folder)
        if (
                "htk" in out_formats
        ...
        if "confidence" in out_formats:
            self.save_confidence_fn(out_folder)
